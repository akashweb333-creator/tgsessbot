...
from referral import (
    setup_referral_handlers,
    set_bot_username,
    process_referral_commission,
    show_referral_menu  # ✅ ADD THIS LINE
)
from whatsapp_handler import (
    get_whatsapp_countries,
    purchase_whatsapp_number,
    monitor_whatsapp_order,
    get_user_whatsapp_purchases,
    get_whatsapp_price
)
...
try:
    ...
    import io
    TON_AVAILABLE = True
    logger.info("✅ TON imports successful")
except ImportError as e:
    TON_AVAILABLE = False
    logger.error(f"❌ TON import failed: {e}")
    import traceback
    traceback.print_exc()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if existing_user and existing_user.get('is_banned', False):
        await update.message.reply_text(
            ...
            "If you believe this is an error, please contact support: @Akash_support_bot",
            parse_mode='Markdown'
        )
        return
    ...
    if context.args and len(context.args) > 0:
        # Extract referral code: /start ref_123456789
        ...
        if ref_code.startswith('ref_'):
            try:
                ...
                logger.warning(f"⚠️ Invalid referral code: {ref_code}")
    ...
    if not existing_user:
        ...
        User.create(
            ...
            referred_by=referrer_id  # Store who referred this user
        )
        logger.info(f"✅ New user created: {user.id}")
        
        # ✅ SEND WELCOME MESSAGE IF REFERRED
        if referrer_id:
            try:
                # Notify referrer
                await context.bot.send_message(
                    ...
                    text=(
                        ...
                        f"💰 You'll earn commission on their purchases!"
                    ),
                    parse_mode='Markdown'
                )
            except Exception as e:
    ...
    # ✅ BUILD MAIN MENU WITH REFERRAL BUTTON
    keyboard = [
        [
        ...
        [
            InlineKeyboardButton("🎁 Referral Program", callback_data='referral_menu')  # ✅ NEW
        ],
        [
...
# ============================================
# MENU FUNCTIONS (Place these BEFORE button_callback)
# ============================================

async def show_buy_options(query):
    """Show buy options - WhatsApp, Telegram, Purchase History"""
    keyboard = [
        [InlineKeyboardButton("📱 WhatsApp", callback_data='buy_whatsapp')],
        [InlineKeyboardButton("📞 Telegram", callback_data='buy_telegram_menu')],
    ...
    await query.edit_message_text(
        "🛒 **Buy Numbers**\n\n"
        "📱 **WhatsApp**: Get WhatsApp verification numbers\n"
        "📞 **Telegram**: Manual OTP or Session files\n"
        "📦 **History**: View your past purchases\n\n"
        "💰 All prices in USD\n\n"
        "Choose a service:",
        reply_markup=reply_markup,
        parse_mode='Markdown'
...
async def show_telegram_submenu(query):
    """Show Telegram submenu - Manual OTP and Sessions"""
    keyboard = [
        [InlineKeyboardButton("📞 Manual OTP", callback_data='buy_manual_otp')],
        [InlineKeyboardButton("📲 Sessions", callback_data='buy_sessions_menu')],
        [InlineKeyboardButton("« Back", callback_data='buy_numbers')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(
        "📞 **Telegram Numbers**\n\n"
        "**📞 Manual OTP**: Get OTP code delivered automatically\n"
        "**📲 Sessions**: Download ready-to-use session files\n\n"
        "Choose your option:",
        reply_markup=reply_markup,
        parse_mode='Markdown'
    )
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle button callbacks - FIXED: Skip conversation patterns entirely"""
    ...
    # ============================================
    # DON'T PROCESS CONVERSATION CALLBACKS AT ALL
    # button_callback is in GROUP 1, ConversationHandlers are in GROUP 0
    # Group 0 should process first, but just to be safe, we skip these
    # ============================================
    ...
    # Also check crypto_now_custom_ prefix
    if query.data in conversation_patterns or \
       query.data.startswith('crypto_now_custom_') or \
       query.data.startswith('bulk_custom_'):
        logger.warning(f"   ⚠️ ERROR: Conversation callback reached button_callback!")
        logger.warning(f"   This should have been caught by ConversationHandler in group 0")
        # Still skip it
        return
    
    # CRITICAL: Always answer callback queries immediately for non-conversation buttons
    try:
        await query.answer()
    except Exception as e:
    ...
    # ============================================
    # REFERRAL HANDLERS - MUST BE FIRST
    # ============================================
    if query.data == 'referral_menu':
        logger.info(f"   → Processing: Referral Menu")
        try:
            from referral import show_referral_menu
            await show_referral_menu(query, user_id)
            logger.info(f"   ✅ Success: Referral menu shown")
        except Exception as e:
            logger.error(f"   ❌ Error in referral_menu: {e}")
            import traceback
            traceback.print_exc()
            await query.edit_message_text(
                f"❌ Error loading referral menu.\n\nError: {str(e)}\n\nPlease contact support."
            )
        return
    
    if query.data == 'referral_stats':
        logger.info(f"   → Processing: Referral Stats")
        try:
            from referral import show_referral_stats
            await show_referral_stats(update, context)
            logger.info(f"   ✅ Success: Referral stats shown")
        except Exception as e:
    ...
    if query.data == 'referral_history':
        logger.info(f"   → Processing: Referral History")
        try:
            from referral import show_referral_history
            await show_referral_history(update, context)
            logger.info(f"   ✅ Success: Referral history shown")
        except Exception as e:
            logger.error(f"   ❌ Error in referral_history: {e}")
            import traceback
            traceback.print_exc()
        return
    ...
    # WHATSAPP HANDLERS
    # ============================================
    if query.data == 'buy_whatsapp':
        logger.info("📱 Processing WhatsApp menu")
        try:
            await show_whatsapp_menu(query)
        except Exception as e:
            logger.error(f"WhatsApp menu error: {e}")
            import traceback
            traceback.print_exc()
        return
    
    if query.data.startswith('whatsapp_country_'):
        country_id = query.data.replace('whatsapp_country_', '')
        logger.info(f"📱 Processing WhatsApp purchase for country: {country_id}")
        try:
            await process_whatsapp_purchase(query, country_id, context)
        except Exception as e:
            logger.error(f"WhatsApp purchase error: {e}")
            import traceback
            traceback.print_exc()
        return
    
    if query.data == 'purchase_history':
        logger.info("📦 Processing purchase history")
        try:
    ...
    # TELEGRAM SUBMENU
    # ============================================
    if query.data == 'buy_telegram_menu':
        logger.info("📞 Processing Telegram submenu")
        try:
            await show_telegram_submenu(query)
        except Exception as e:
            logger.error(f"Telegram submenu error: {e}")
            import traceback
            traceback.print_exc()
        return
    ...
    # REFUND HANDLERS
    # ============================================
    if query.data.startswith('refund_confirm_'):
        await handle_refund_confirm(query, context)
        return
        
    if query.data.startswith('refund_reject_'):
        await handle_refund_reject(query, context)
        return
    
    # ============================================
    # MAIN MENU NAVIGATION - WRAPPED IN TRY-EXCEPT
    # ============================================
    try:
        if query.data == 'balance':
            ...
            logger.info("💳 Showing payment methods")
            await show_payment_method_choice(query)
            
        elif query.data == 'buy_numbers':
            logger.info("🛒 Showing buy options")
            ...
            logger.info("💬 Showing support")
            await show_support(query)
            
        elif query.data == 'my_purchases':
            logger.info("📦 Showing purchases")
            ...
            logger.info("🔙 Back to buy options")
            await show_buy_options(query)
        ...
        # ============================================
        # PAYMENT METHODS - TON FIXED HERE
        # ============================================
        elif query.data == 'deposit_ton':
            logger.info(f"💎 TON deposit clicked, TON_AVAILABLE={TON_AVAILABLE}")
            if TON_AVAILABLE:
                logger.info("💎 Calling show_ton_deposit_options")
                try:
                    await show_ton_deposit_options(query)
                    logger.info("✅ TON deposit options shown")
                except Exception as e:
                    logger.error(f"❌ Error in show_ton_deposit_options: {e}")
                    import traceback
                    traceback.print_exc()
                    await query.edit_message_text(
                        "❌ Error loading TON deposit options.\n\n"
                        "Please try again or contact support."
                    )
            else:
                logger.warning("⚠️ TON not available")
                await query.edit_message_text(
                    "❌ TON payments are currently unavailable.\n\n"
            ...
            logger.info("₹ INR deposit")
            await show_inr_deposit_options(query)
            
        elif query.data == 'deposit_crypto_manual':
            logger.info("💰 Crypto manual")
            await show_crypto_manual_selection(query)
        
        # ============================================
        # NOWPAYMENTS - CRYPTO CURRENCY SELECTION
        # ============================================
        elif query.data.startswith('crypto_now_') and not query.data.startswith('crypto_now_pay_'):
            # Don't handle crypto_now_custom_ here - let ConversationHandler handle it
            if not query.data.startswith('crypto_now_custom_'):
        ...
        # ============================================
            ...
            logger.info(f"💎 Processing TON deposit: ${amount}")
        ...
        # ============================================
            ...
            logger.warning(f"   ⚠️ Unhandled callback: {query.data}")
            
    except Exception as e:
        logger.error(f"❌ ERROR in button_callback for {query.data}: {e}")
        import traceback
        traceback.print_exc()
...
async def ask_custom_deposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start custom amount conversation - FIXED SIGNATURE"""
    query = update.callback_query
    await query.answer()
...
async def receive_custom_deposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle custom amount input - FIXED: Removed generate_ton_payment_url"""
    try:
        ...
        # ✅ FIXED: TON payment - Use process_ton_deposit approach
        if payment_method == 'ton':
            ...
            # Create payment info using existing method
            payment_info = ton_handler.create_deposit_address(update.effective_user.id, amount)
            
            if not payment_info.get('success'):
                await update.message.reply_text(
                    "❌ Error creating TON payment. Please try again later."
                )
                return ConversationHandler.END
...
async def process_bulk_purchase(query, country, quantity, context):
    """Process bulk session purchase - WITH NEW DISCOUNTS AND REFERRAL"""
    ...
    try:
        ...
        # ✅ PROCESS REFERRAL COMMISSION FOR BULK
        try:
            await process_referral_commission(user_id, total_cost)
            logger.info(f"✅ Bulk referral commission processed: ${total_cost}")
        except Exception as e:
            logger.error(f"❌ Bulk referral commission error: {e}")
...
async def process_session_purchase(query, user_id, session_id, context):
    """
    Process session purchase - WITH ADMIN NOTIFICATION AND REFERRAL
    """
    ...
    try:
        ...
        purchase_amount = session['price'] if session else 0.0
        
        # Process commission (Level 1: 3%, Level 2: 1.5%)
        await process_referral_commission(user_id, purchase_amount)
        logger.info(f"✅ Referral commission processed for user {user_id}")
    except Exception as e:
...
async def process_manual_purchase(query, user_id, session_id, context):
    """Process manual OTP purchase - WITH ADMIN NOTIFICATION AND REFERRAL"""
    await query.edit_message_text("⏳ Processing your purchase...")
    ...
    try:
        ...
        # Process commission
        await process_referral_commission(user_id, purchase_amount)
        logger.info(f"✅ Manual OTP referral commission: ${purchase_amount}")
    except Exception as e:
        logger.error(f"❌ Manual referral commission error: {e}")
    ...
    # Start OTP listener (existing code continues...)
    try:
        message_id = int(result['session_string'])
...
async def show_whatsapp_menu(query):
    """Show WhatsApp purchase menu"""
    try:
        countries = await get_whatsapp_countries()
        
        if not countries:
            keyboard = [[InlineKeyboardButton("« Back", callback_data='buy_numbers')]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(
                "❌ No WhatsApp numbers available at the moment.\n\n"
                "Please check back later.",
                reply_markup=reply_markup
            )
            return
        ...
        for country in countries:
            if country['available']:
                price = get_whatsapp_price(country['id'])
                keyboard.append([InlineKeyboardButton(
                    f"🌍 {country['name']} - ${price:.2f}",
                    callback_data=f'whatsapp_country_{country["id"]}'
                )])
        
        keyboard.append([InlineKeyboardButton("« Back", callback_data='buy_numbers')])
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text(
            "📱 **WhatsApp Numbers**\n\n"
            "✅ Instant delivery\n"
            "✅ Best operators auto-selected\n"
...
async def process_whatsapp_purchase(query, country_id, context):
    """Process WhatsApp number purchase - FIXED DISPLAY"""
    user_id = query.from_user.id
    ...
    try:
        # Get price
        price = get_whatsapp_price(country_id)
        
        # Purchase number
        order, error = await purchase_whatsapp_number(user_id, country_id, price)
        
        if error:
            keyboard = [[InlineKeyboardButton("« Back", callback_data='buy_whatsapp')]]
            reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        # ✅ FIXED: Format phone number properly (add + if not present)
        # ✅ NEW (cleaner):
        from whatsapp_handler import format_phone_number
        phone_number = format_phone_number(order['phone_number'])
        
        # ✅ FIXED: Show ONLY the complete phone number
        # The +84 is already part of the number, no need to show separately
        waiting_msg = await query.edit_message_text(
            "⏳ **WhatsApp Number Purchased!**\n\n"
            f"📱 Number: `{phone_number}`\n"
            f"🌍 Country: {order['country_name']}\n\n"
            "Waiting for OTP code...\n"
            "⏰ Auto-refund in 5 minutes if no OTP\n\n"
            "Please wait...",
            parse_mode='Markdown'
        )
        ...
        try:
            await context.bot.send_message(
                chat_id=config.OWNER_ID,
                text=(
                    "📱 **NEW WHATSAPP PURCHASE**\n\n"
                    f"👤 User: @{user.get('username', 'Unknown')} (`{user_id}`)\n"
                    f"📱 Number: `{phone_number}`\n"
                    f"🌍 Country: {order['country_name']}\n"
                    f"💰 Price: ${price:.2f}\n"
        ...
        asyncio.create_task(monitor_whatsapp_order(
            context.bot,
            user_id,
            order['order_id'],
...
async def show_purchase_history(query, user_id):
    """Show unified purchase history - Telegram + WhatsApp"""
    try:
        # Get Telegram purchases
        telegram_purchases = await get_user_purchases(user_id)
        
        # Get WhatsApp purchases
        whatsapp_purchases = await get_user_whatsapp_purchases(user_id)
        
        if not telegram_purchases and not whatsapp_purchases:
            keyboard = [
                [InlineKeyboardButton("🛒 Buy Numbers", callback_data='buy_numbers')],
                [InlineKeyboardButton("« Back", callback_data='back_menu')]
            ]
            ...
            await query.edit_message_text(
                "📦 No purchases yet.\n\n"
                "Start by purchasing WhatsApp or Telegram numbers!",
                reply_markup=reply_markup
            )
            return
        ...
        # Show WhatsApp purchases
        if whatsapp_purchases:
            message += "📱 **WhatsApp Numbers:**\n\n"
            for i, purchase in enumerate(whatsapp_purchases[:10], 1):
                message += f"{i}. 📱 {purchase['phone_number']}\n"
                message += f"   🌍 {purchase['country_name']}\n"
                message += f"   🔑 OTP: `{purchase.get('otp_code', 'N/A')}`\n"
                message += f"   📅 {purchase['created_at'].strftime('%Y-%m-%d %H:%M')}\n\n"
        
        # Show Telegram purchases
        if telegram_purchases:
            message += "📞 **Telegram Numbers:**\n\n"
            for i, purchase in enumerate(telegram_purchases[:10], 1):
                message += f"{i}. 📱 {purchase.phone_number}\n"
        ...
        total = len(whatsapp_purchases) + len(telegram_purchases)
        message += f"Total purchases: {total}"
        ...
        import traceback
        traceback.print_exc()
        await query.edit_message_text("❌ Error loading purchase history")
...
async def show_ton_deposit_options(query):
    """Show TON deposit amount options - COMPLETELY FIXED"""
    logger.info("🔹 show_ton_deposit_options called")
    
    try:
        # Check if TON is available
        if not TON_AVAILABLE:
            logger.warning("❌ TON_AVAILABLE is False")
        ...
        for amount in amounts:
            if amount >= min_deposit:
                row.append(InlineKeyboardButton(
                    f"${amount} USD", 
                    callback_data=f'ton_deposit_{amount}'
                ))
                if len(row) == 2:
                    keyboard.append(row)
                    row = []
        ...
        logger.info("📤 Sending TON deposit options message")
        ...
        logger.info("✅ TON deposit options sent successfully")
        
    except Exception as e:
        logger.error(f"❌ CRITICAL ERROR in show_ton_deposit_options: {e}")
        import traceback
        traceback.print_exc()
...
async def handle_refund_confirm(query, context):
    """Handle admin confirming refund"""
    try:
        # Parse callback data: refund_confirm_ORDER_ID_USER_ID_MESSAGE_ID
        parts = query.data.split('_')
        order_id = parts[2]
        user_id = int(parts[3])
        user_message_id = int(parts[4])
        
        # Process refund
        success = await cancel_and_refund_order(order_id, reason="admin_confirmed")
        ...
        if success:
            ...
            if order:
                ...
                try:
                    ...
                    logger.error(f"Could not notify user about refund: {e}")
                
                # Update admin message
                await query.edit_message_text(
                    f"{query.message.text}\n\n"
                    f"✅ **REFUND APPROVED**\n"
                    f"💰 ${order['price']:.2f} refunded to user",
                    parse_mode='Markdown'
                )
            else:
                await query.answer("❌ Order not found", show_alert=True)
        else:
            await query.answer("❌ Refund failed. Check logs.", show_alert=True)
            
    except Exception as e:
        logger.error(f"Refund confirm error: {e}")
        await query.answer(f"❌ Error: {str(e)}", show_alert=True)


async def handle_refund_reject(query, context):
    """Handle admin rejecting refund"""
    try:
        # Parse callback data: refund_reject_ORDER_ID_USER_ID_MESSAGE_ID
        parts = query.data.split('_')
        order_id = parts[2]
        user_id = int(parts[3])
        user_message_id = int(parts[4])
            ...
            try:
                await context.bot.edit_message_text(
                    ...
                    text=(
                        ...
                        "Please contact @Akash_support_bot for assistance."
                    ),
                    parse_mode='Markdown'
                )
            except Exception as e:
                logger.error(f"Could not notify user about rejection: {e}")
        ...
        logger.error(f"Refund reject error: {e}")
        await query.answer(f"❌ Error: {str(e)}", show_alert=True)

def safe_mongo_operation(operation, default=None):
    """Safely execute MongoDB operation with retry"""
    try:
        return operation()
    except Exception as e:
...
"""
FIXED MAIN FUNCTION - Resolves "Event loop is closed" error
Replace your main() function with this
"""

def main():
    """Start the bot - FIXED HANDLER REGISTRATION ORDER"""
    # ============================================
    # START WEB SERVER
    # ============================================
    keep_alive()
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # ✅ SET BOT USERNAME FOR REFERRAL SYSTEM
            try:
                set_bot_username(bot_info['username'])
                logger.info(f"✅ Bot username set for referrals: @{bot_info['username']}")
            except Exception as e:
                logger.error(f"⚠️ Could not set bot username: {e}")
        else:
    ...
    # ============================================
    # CONVERSATION HANDLERS (GROUP 0 - HIGHEST PRIORITY)
    # These MUST be registered BEFORE the main button_callback
    # ============================================
    
    # Custom deposit conversation
    ...
    application.add_handler(deposit_conv, group=0)  # ✅ GROUP 0 = HIGHEST PRIORITY
    logger.info("✅ Deposit conversation registered (group=0)")
    
    # Bulk custom quantity conversation
    ...
    application.add_handler(bulk_custom_conv, group=0)  # ✅ GROUP 0
    logger.info("✅ Bulk custom conversation registered (group=0)")
    
    # ============================================
    # SETUP MODULE HANDLERS (GROUP 0)
    # ============================================
    ...
    try:
        ...
        logger.info("✅ Leader handlers registered")
    except Exception as e:
        logger.error(f"❌ Leader handler error: {e}")
        import traceback
        traceback.print_exc()
    ...
    try:
        ...
        logger.error(f"❌ Seller handler error: {e}")
        import traceback
        traceback.print_exc()
    
    # 4. ✅ REFERRAL HANDLERS (CRITICAL)
    try:
        setup_referral_handlers(application)
    ...
    # ============================================
    # ADMIN COMMAND HANDLERS (GROUP 0)
    # ============================================
    application.add_handler(CommandHandler("pending_sellers", admin_pending_sellers), group=0)
    application.add_handler(CommandHandler("pending_withdrawals", admin_pending_withdrawals), group=0)
    application.add_handler(CommandHandler("pending_ton", admin_pending_ton), group=0)
    application.add_handler(CommandHandler("credit_ton", admin_credit_ton_manual), group=0)
    application.add_handler(CommandHandler("verify_crypto", admin_verify_crypto), group=0)
    application.add_handler(CommandHandler("pending_crypto", admin_pending_crypto), group=0)
    logger.info("✅ Admin commands registered (group=0)")
    
    # ============================================
    # SPECIFIC CALLBACK HANDLERS (GROUP 0)
    # ============================================
    application.add_handler(CallbackQueryHandler(
        handle_admin_credit_callback, 
        pattern='^admin_credit_confirm_'
    ), group=0)
    application.add_handler(CallbackQueryHandler(
        handle_admin_credit_callback, 
        pattern='^admin_cancel$'
    ), group=0)
    logger.info("✅ Specific callback handlers registered (group=0)")
    
    # ============================================
    # ✅ MAIN CALLBACK HANDLER (GROUP 1 - LOWER PRIORITY)
    # This catches ALL remaining callbacks that weren't caught by specific handlers
    # MUST be registered AFTER all other handlers with group=1
    # ============================================
    application.add_handler(CallbackQueryHandler(button_callback), group=1)
    logger.info("✅ Main callback handler registered (group=1 - catches remaining callbacks)")
    ...
    # ============================================
    # START POLLING (SIMPLE, NO MANUAL EVENT LOOP)
    # ============================================
    try:
