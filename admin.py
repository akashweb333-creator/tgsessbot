...
"""
from referral import admin_referral_stats
import os
import logging
import zipfile
import io
...
from whatsapp_handler import (
    ...
    get_all_whatsapp_settings,
    update_whatsapp_price
)
...
(UPLOAD_SESSION, UPLOAD_BULK, GET_COUNTRY, GET_PRICE, GET_2FA, 
 ...
 DELETE_SESSION_CONFIRM, BAN_USER_ID, BAN_USER_CONFIRM,
 REMOVE_BALANCE_USER, REMOVE_BALANCE_AMOUNT, REMOVE_BALANCE_CONFIRM,
 UNBAN_USER_ID, UNBAN_USER_CONFIRM) = range(26)

EDIT_WHATSAPP_PRICE = 100
...
async def admin_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin menu - UPDATED WITH BAN/UNBAN USER AND REMOVE BALANCE"""
    keyboard = [
        [
        ...
        [
            InlineKeyboardButton("🗑️ Delete Sessions", callback_data='admin_delete_sessions'),
...
@admin_only
async def test_whatsapp_api(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Test TemporaSMS API connection with detailed diagnostics"""
    await update.message.reply_text("⏳ Testing TemporaSMS API connection...")
    
    try:
        from whatsapp_handler import temporasms
        
        message = "📊 **TemporaSMS API Test Results**\n\n"
        all_passed = True
        
        # ============================================
        # Test 1: Get Balance
        # ============================================
        message += "**Test 1: Balance Check**\n"
        try:
            balance_result = temporasms.get_balance()
            
            logger.info(f"Balance result: {balance_result}")
            
            if balance_result.get('status') == 'error':
                message += f"❌ Failed\n"
                message += f"└─ Error: {balance_result.get('message', 'Unknown error')}\n"
                all_passed = False
            else:
                balance = balance_result.get('balance', 'N/A')
                message += f"✅ Success\n"
                message += f"└─ Balance: ${balance}\n"
                
                # Warn if low balance
                if isinstance(balance, (int, float)) and balance < 5.0:
                    message += f"└─ ⚠️ Low balance! Add more funds.\n"
        except Exception as e:
            message += f"❌ Exception\n"
        ...
        # ============================================
        message += "**Test 2: Countries Check**\n"
        try:
            countries_result = temporasms.get_countries()
            
            logger.info(f"Countries result: {countries_result}")
            
            if countries_result.get('status') == 'error':
                message += f"❌ Failed\n"
                message += f"└─ Error: {countries_result.get('message', 'Unknown error')}\n"
                all_passed = False
            else:
                ...
                if isinstance(countries_result, dict) and 'countries' in countries_result:
                    ...
                    message += f"✅ Success\n"
                    message += f"└─ Found {len(countries)} countries\n"
                elif isinstance(countries_result, list):
                    message += f"✅ Success\n"
                    message += f"└─ Found {len(countries_result)} countries\n"
                elif isinstance(countries_result, dict):
                    message += f"✅ Success\n"
                    message += f"└─ Response: {str(countries_result)[:100]}\n"
                else:
                    message += f"⚠️ Unexpected format\n"
                    message += f"└─ Type: {type(countries_result)}\n"
        except Exception as e:
            message += f"❌ Exception\n"
            message += f"└─ Error: {str(e)[:100]}\n"
            all_passed = False
        ...
        if all_passed:
            ...
            message += "✅ TemporaSMS API is working correctly"
        else:
            message += "━━━━━━━━━━━━━━━━\n"
        ...
        await update.message.reply_text(
            "❌ **Import Error**\n\n"
            "Could not import whatsapp_handler module.\n"
...
async def admin_whatsapp_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show WhatsApp settings - Country prices"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        # Get all WhatsApp country settings
        settings = get_all_whatsapp_settings()
        ...
        for setting in settings:
            ...
            message += f"🌍 **{country_name}**: ${price:.2f}\n"
            
            keyboard.append([InlineKeyboardButton(
                f"✏️ Edit {country_name} Price",
                callback_data=f'admin_wa_edit_{country_id}'
            )])
        ...
        logger.error(f"WhatsApp settings error: {e}")
        import traceback
        traceback.print_exc()
        await query.edit_message_text(f"❌ Error: {str(e)}")


async def admin_whatsapp_edit_price_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Start editing WhatsApp country price"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        # Extract country_id from callback data
        country_id = query.data.replace('admin_wa_edit_', '')
        
        # Store in context
        context.user_data['wa_edit_country_id'] = country_id
        
        # Get country details
        settings = get_all_whatsapp_settings()
        country = next((s for s in settings if s['country_id'] == country_id), None)
        ...
        await query.edit_message_text(
            f"✏️ **Edit WhatsApp Price**\n\n"
            f"🌍 Country: **{country['country_name']}**\n"
            f"💰 Current Price: **${country['price']:.2f}**\n\n"
            f"Enter new price in USD:\n"
            f"Example: 0.75\n\n"
            f"Send /cancel to abort.",
            parse_mode='Markdown'
        )
        
        return EDIT_WHATSAPP_PRICE
        
    except Exception as e:
        logger.error(f"Edit price start error: {e}")
        await query.edit_message_text(f"❌ Error: {str(e)}")
        return ConversationHandler.END


async def admin_whatsapp_receive_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receive new WhatsApp price - FIXED"""
    if update.effective_user.id != config.OWNER_ID:
        await update.message.reply_text("❌ Unauthorized")
        return ConversationHandler.END
    ...
    try:
        ...
        country_id = context.user_data.get('wa_edit_country_id')
        ...
        old_price = country.get('price', 0.0)
        
        # ✅ Update price in database
        success = update_whatsapp_price(country_id, new_price)
        
        if success:
            # ✅ Send success message with keyboard
            keyboard = [
                [InlineKeyboardButton("📱 WhatsApp Settings", callback_data='admin_whatsapp_settings')],
            ...
            await update.message.reply_text(
                ...
                f"This will apply to all new WhatsApp purchases.",
                reply_markup=reply_markup,
                parse_mode='Markdown'
            )
        else:
            await update.message.reply_text(
                ...
                "If this persists, contact @Akash_support_bot"
            )
        
        # Clear context
        context.user_data.pop('wa_edit_country_id', None)
        ...
        logger.error(f"Receive price error: {e}")
        import traceback
        traceback.print_exc()
        await update.message.reply_text(f"❌ Error: {str(e)}")
        return ConversationHandler.END

async def handle_whatsapp_refund_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle admin refund confirmation"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        if query.data.startswith('whatsapp_refund_confirm_'):
            # Admin confirms refund
            db_order_id_str = query.data.replace('whatsapp_refund_confirm_', '')
            db_order_id = ObjectId(db_order_id_str)
            ...
            # Execute refund
            success, message = await admin_confirm_refund(order['order_id'], db_order_id)
            
            if success:
                user = User.get_by_telegram_id(order['user_id'])
                ...
                try:
                    await context.bot.send_message(
                        ...
                        text=(
                            "✅ **Refund Processed**\n\n"
                            f"Your refund of ${order['price']:.2f} has been processed.\n"
                            f"💳 New Balance: ${user['balance']:.2f}\n\n"
                ...
                await query.edit_message_text(f"❌ Refund failed: {message}")
        
        elif query.data.startswith('whatsapp_refund_cancel_'):
            # Admin cancels refund
            db_order_id_str = query.data.replace('whatsapp_refund_cancel_', '')
            db_order_id = ObjectId(db_order_id_str)
            ...
            # Mark as cancelled without refund
            database.whatsapp_orders.update_one(
                {'_id': db_order_id},
                {'$set': {'status': 'cancelled_no_refund'}}
            )
        ...
        logger.error(f"Refund callback error: {e}")
        import traceback
        traceback.print_exc()
        await query.edit_message_text(f"❌ Error: {str(e)}")
...
async def admin_ban_user_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "(You can find it in the user list)\n\n"
...
async def receive_ban_user_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user_id == config.OWNER_ID:
            await update.message.reply_text("❌ Cannot ban admin user!")
            return BAN_USER_ID
        ...
        context.user_data['ban_username'] = user.get('username')
        
        keyboard = [
            [InlineKeyboardButton("✅ Confirm Ban", callback_data='admin_ban_confirm')],
            [InlineKeyboardButton("❌ Cancel", callback_data='admin_ban_cancel')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        await update.message.reply_text(
            ...
            f"Username: @{user.get('username') or 'N/A'}\n"
            f"Balance: ${user['balance']:.2f}\n\n"
            "This user will be banned and cannot use the bot.\n\n"
            "Confirm?",
            reply_markup=reply_markup
        )
        
        return BAN_USER_CONFIRM
            
    except ValueError:
        await update.message.reply_text("❌ Invalid User ID. Please send a valid number.")
        return BAN_USER_ID
    except Exception as e:
        logger.error(f"Error in receive_ban_user_id: {e}")
        await update.message.reply_text(f"❌ Error: {str(e)}")
        return BAN_USER_ID
...
async def confirm_ban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'admin_ban_cancel':
        context.user_data.clear()
    ...
    try:
        ...
        if result.modified_count > 0:
            ...
            try:
                ...
                logger.error(f"Could not notify banned user: {e}")
        else:
        ...
        import traceback
        traceback.print_exc()
        await query.edit_message_text(f"❌ Error banning user: {str(e)}")
    finally:
        context.user_data.clear()
...
async def receive_remove_balance_user_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user['balance'] <= 0:
            await update.message.reply_text(
                f"⚠️ User {user_id} has no balance to remove.\n"
                f"Current balance: ${user['balance']:.2f}"
            )
            return REMOVE_BALANCE_USER
        
        context.user_data['remove_balance_user_id'] = user_id
        context.user_data['remove_balance_username'] = user.get('username')
        context.user_data['remove_current_balance'] = user['balance']
        ...
        await update.message.reply_text(
            ...
            f"Username: @{user.get('username') or 'N/A'}\n"
            f"Current Balance: ${user['balance']:.2f}\n\n"
            "Now send the amount to remove (in USD):"
        )
...
async def receive_remove_balance_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        user_id = context.user_data['remove_balance_user_id']
        current_balance = context.user_data['remove_current_balance']
        new_balance = max(0, current_balance - amount)  # Don't go negative
        actual_removed = current_balance - new_balance
        
        keyboard = [
            [InlineKeyboardButton("✅ Confirm", callback_data='admin_remove_balance_confirm')],
            [InlineKeyboardButton("❌ Cancel", callback_data='admin_remove_balance_cancel')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        context.user_data['remove_balance_amount'] = amount
        context.user_data['actual_removed'] = actual_removed
        
        warning = ""
        if amount > current_balance:
            warning = f"\n⚠️ Amount exceeds balance! Will remove ${actual_removed:.2f} (full balance)\n"
        
        await update.message.reply_text(
            f"💸 Confirm Balance Removal\n\n"
            f"User ID: {user_id}\n"
            f"Username: @{context.user_data.get('remove_balance_username') or 'N/A'}\n"
            f"Current Balance: ${current_balance:.2f}\n"
            f"Amount to Remove: ${amount:.2f}\n"
            f"New Balance: ${new_balance:.2f}\n"
            f"{warning}\n"
            "Confirm?",
            reply_markup=reply_markup
        ...
        return REMOVE_BALANCE_CONFIRM
        
    except ValueError:
        await update.message.reply_text("❌ Invalid amount. Please send a valid number.")
        return REMOVE_BALANCE_AMOUNT
    except Exception as e:
        logger.error(f"Error in receive_remove_balance_amount: {e}")
        await update.message.reply_text(f"❌ Error: {str(e)}")
        return REMOVE_BALANCE_AMOUNT

async def confirm_remove_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Confirm and remove balance"""
    query = update.callback_query
    await query.answer()
    ...
    if query.data == 'admin_remove_balance_cancel':
        context.user_data.clear()
        await query.edit_message_text("❌ Balance removal cancelled.")
        return ConversationHandler.END
    
    user_id = context.user_data['remove_balance_user_id']
    amount = context.user_data['remove_balance_amount']
    
    try:
        # Remove balance using MongoDB method
        success = User.update_balance(user_id, amount, operation='subtract')
        ...
        if success:
            ...
            try:
                await context.bot.send_message(
                    chat_id=user_id,
                    text=f"⚠️ Balance Update\n\nAmount Removed: ${amount:.2f}\nNew Balance: ${user['balance']:.2f}"
                )
            except Exception as e:
                logger.error(f"Could not notify user: {e}")
        else:
        ...
        import traceback
        traceback.print_exc()
        await query.edit_message_text(f"❌ Error removing balance: {str(e)}")
    finally:
        context.user_data.clear()
...
async def admin_unban_user_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "(You can find it in the user list)\n\n"
        "This will restore user's access to the bot.\n\n"
        "Send /cancel to abort."
    )
...
async def receive_unban_user_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        context.user_data['unban_username'] = user.get('username')
        
        keyboard = [
            [InlineKeyboardButton("✅ Confirm Unban", callback_data='admin_unban_confirm')],
            [InlineKeyboardButton("❌ Cancel", callback_data='admin_unban_cancel')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        banned_at = user.get('banned_at', 'Unknown')
        if isinstance(banned_at, datetime):
            banned_at = banned_at.strftime('%Y-%m-%d %H:%M')
        
        await update.message.reply_text(
            f"✅ Confirm User Unban\n\n"
            f"User ID: {user_id}\n"
            f"Username: @{user.get('username') or 'N/A'}\n"
            f"Balance: ${user['balance']:.2f}\n"
            f"Banned Since: {banned_at}\n\n"
            "This user will be unbanned and can use the bot again.\n\n"
            "Confirm?",
            reply_markup=reply_markup
        )
        
        return UNBAN_USER_CONFIRM
            
    except ValueError:
        await update.message.reply_text("❌ Invalid User ID. Please send a valid number.")
        return UNBAN_USER_ID
    except Exception as e:
        logger.error(f"Error in receive_unban_user_id: {e}")
        await update.message.reply_text(f"❌ Error: {str(e)}")
        return UNBAN_USER_ID

async def confirm_unban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Confirm and unban user"""
    query = update.callback_query
    await query.answer()
    ...
    if query.data == 'admin_unban_cancel':
        context.user_data.clear()
        await query.edit_message_text("❌ Unban cancelled.")
        return ConversationHandler.END
    
    user_id = context.user_data['unban_user_id']
    ...
    try:
        ...
        if result.modified_count > 0:
            ...
            try:
                ...
                logger.error(f"Could not notify unbanned user: {e}")
        else:
        ...
        import traceback
        traceback.print_exc()
        await query.edit_message_text(f"❌ Error unbanning user: {str(e)}")
    finally:
        context.user_data.clear()
...
async def admin_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Go back to admin menu - WITH BAN/UNBAN USER AND REMOVE BALANCE"""
    query = update.callback_query
    await query.answer()
...
async def admin_settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show admin settings menu - WITH WHATSAPP"""
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        try:
            ...
            ton_rate = ton_handler.usd_to_ton(1.0)
        except:
            ton_rate = 0.5
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("📱 WhatsApp Settings", callback_data='admin_whatsapp_settings')],  # ✅ NEW
        ...
        await query.edit_message_text(
            ...
            f"📱 **WhatsApp:** Manage country prices\n\n"
...
def setup_admin_handlers(application):
    ...
    application.add_handler(CommandHandler("test_whatsapp", test_whatsapp_api))
    # ============================================================================
    ...
    whatsapp_settings_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(admin_whatsapp_edit_price_start, pattern='^admin_wa_edit_')
        ],
        states={
            EDIT_WHATSAPP_PRICE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, admin_whatsapp_receive_price)
            ]
        },
        fallbacks=[CommandHandler('cancel', cancel_operation)],
        allow_reentry=True
    )
    application.add_handler(whatsapp_settings_conv)
    
    # ✅ ADD WHATSAPP CALLBACK HANDLERS
    application.add_handler(CallbackQueryHandler(
        admin_whatsapp_settings,
        pattern='^admin_whatsapp_settings$'
    ))
    
    application.add_handler(CallbackQueryHandler(
        handle_whatsapp_refund_callback,
        pattern='^whatsapp_refund_'
    ))
    
    logger.info("✅ WhatsApp admin handlers registered")
    ...
    ban_user_conv = ConversationHandler(
        ...
        states={
            ...
            BAN_USER_CONFIRM: [CallbackQueryHandler(confirm_ban_user, pattern='^admin_ban_')]
        },
        fallbacks=[CommandHandler('cancel', cancel_operation)],
        allow_reentry=True
    )
    application.add_handler(ban_user_conv)
    logger.info("✅ Ban user conversation handler registered")
    ...
    unban_user_conv = ConversationHandler(
        ...
        states={
            ...
            UNBAN_USER_CONFIRM: [CallbackQueryHandler(confirm_unban_user, pattern='^admin_unban_')]
        },
        fallbacks=[CommandHandler('cancel', cancel_operation)],
        allow_reentry=True
    )
    application.add_handler(unban_user_conv)
    logger.info("✅ Unban user conversation handler registered")
    ...
    remove_balance_conv = ConversationHandler(
        ...
        states={
            REMOVE_BALANCE_USER: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_remove_balance_user_id)],
            REMOVE_BALANCE_AMOUNT: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_remove_balance_amount)],
            REMOVE_BALANCE_CONFIRM: [CallbackQueryHandler(confirm_remove_balance, pattern='^admin_remove_balance_')]
        },
        fallbacks=[CommandHandler('cancel', cancel_operation)],
        allow_reentry=True
    )
    application.add_handler(remove_balance_conv)
    logger.info("✅ Remove balance conversation handler registered")
